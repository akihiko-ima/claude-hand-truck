...
| ---------- | ---------- | -------------------------------- | ------------------------------------------------ |
| Python     | 3.12.x     | 実行環境                         | -                                                |
| OpenCV     | 4.x        | カメラ映像処理                   | 画像処理の豊富な機能セットと実績                 |
| mediapipe  |            | 手座標検出                       | 手の骨格座標検出                                 |
| NumPy      | 2.x        | センサーデータ数値処理・配列演算 | 高速な数値演算ライブラリ                         |
| matplotlib | 3.x        | センサーエリアマップ画像生成     | ヒートマップ・カラーマップ生成が容易             |
| seaborn    |            | センサーエリアマップ画像生成     | matplotlibで要件達成できない場合                 |
| httpx      | 0.27.x     | APIクライアント（非同期対応）    | 非同期HTTPクライアント。リトライ処理の実装が容易 |
| uv         | 0.5.x      | パッケージ管理                   | -                                                |
